...
export const useInvitationListMyStand = (fairID: string) => {
  ...
  const SWR_KEY_INVITATION_LIST_MY_STANDS = `/invitations/form/stands/${fairID}}`;

  const { isLoading, mutate } = useSWR(
    SWR_KEY_INVITATION_LIST_MY_STANDS,
    async () => await getInvitationStandFormRequest(fairID),
    {
      ...
      onError(error) {
        toastDismiss(SWR_KEY_INVITATION_LIST_MY_STANDS);
        toast(error, {
          type: "error",
          toastId: SWR_KEY_INVITATION_LIST_MY_STANDS,
        });
      },
    }
  );
